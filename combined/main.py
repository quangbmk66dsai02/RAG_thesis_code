...
def main():
    ...
    # Loading Llama-3.2-3B-Instruct model with LoRA adapters and tokenizer
    base_model = AutoModelForCausalLM.from_pretrained(
    'meta-llama/Llama-3.2-3B-Instruct',
    torch_dtype=torch.float16,
    device_map="auto"
)
    model = PeftModel.from_pretrained(base_model, 'llama_components/lora-adapter')
    model = PeftModel.from_pretrained(model, 'llama_components/lora-adapter2').to("cuda")
